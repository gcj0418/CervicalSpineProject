...
class DecNet(nn.Module):
    def __init__(self, heads, final_kernel, head_conv, channel):
        ...
        for head in self.heads:
            ...
            if 'hm' in head:
                last_layer = fc[-1]
                if isinstance(last_layer, nn.Conv2d) and last_layer.bias is not None:
                    nn.init.constant_(last_layer.bias, -2.19)
            else:
                self.fill_fc_weights(fc)

            self.head_layers[head] = fc
    ...
    def forward(self, x):
        ...
        dec_dict = {}
        for head in self.heads:
            head_layer = self.head_layers[head]
            dec_dict[head] = head_layer(c2_combine)
            if 'hm' in head:
