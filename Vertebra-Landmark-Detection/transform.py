...
def resize_with_letterbox(img, pts, dsize, fill=0):
    ...
    if img.ndim == 2:
        canvas = np.full((target_h, target_w), fill, dtype=resized.dtype)
    else:
        canvas = np.full((target_h, target_w, img.shape[2]), fill, dtype=resized.dtype)
...
class Resize(object):
    ...
    def __call__(self, img, pts):
        img, pts = resize_with_letterbox(img, pts, self.dsize, fill=0)
        return img, np.asarray(pts)


class RandomAffine(object):
    def __init__(self, max_rotate=10.0, scale_range=(0.9, 1.1), translate_frac=0.05):
    ...
    def __call__(self, img, pts):
        ...
        tx = random.uniform(-self.translate_frac, self.translate_frac) * w
        ty = random.uniform(-self.translate_frac, self.translate_frac) * h
        ...
        pts_trans = (M.dot(pts_h.T)).T
        return warped, pts_trans
