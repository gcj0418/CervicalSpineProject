...
def load_loss_history(path):
    ...
    if np.size(values) == 0:
        return []
    return np.atleast_1d(values).astype(np.float32).tolist()


def resolve_resume_path(save_path, resume):
    if not resume:
        return ""
    if os.path.isabs(resume):
        return resume
    return os.path.join(save_path, resume)
...
class Network(object):
    ...
    def train_network(self, args):
        ...
        resume_path = resolve_resume_path(save_path, getattr(args, 'resume', ''))
        start_epoch = 1
        train_loss = []
        val_loss = []
        if resume_path and os.path.exists(resume_path):
            self.model = self.load_model(self.model, resume_path, strict=True)
            checkpoint = torch.load(resume_path, map_location=lambda storage, loc: storage)
            start_epoch = int(checkpoint.get('epoch', 0)) + 1
            train_loss = load_loss_history(os.path.join(save_path, 'train_loss.txt'))
            val_loss = load_loss_history(os.path.join(save_path, 'val_loss.txt'))
        ...
        if start_epoch > args.num_epoch:
            print('Resume checkpoint is already beyond the requested target epoch; nothing to train.')
            return
        ...
        # Always persist the final checkpoint so short exploration runs remain usable.
        self.save_model(os.path.join(save_path, 'model_last.pth'), args.num_epoch, self.model)
    ...
    def run_epoch(self, phase, data_loader, criterion):
        ...
        max_batches = 0
        if phase == 'train':
            max_batches = int(getattr(self.args, 'max_train_batches', 0) or 0)
        elif phase == 'val':
            max_batches = int(getattr(self.args, 'max_val_batches', 0) or 0)
        for data_dict in data_loader:
            ...
            if max_batches:
                ...
                if max_batches <= 0:
                    break
        epoch_loss = running_loss / max(batches_seen, 1)
