...
class BaseDataset(data.Dataset):
    def __init__(self, data_dir, phase, input_h=None, input_w=None, down_ratio=4, max_points=56, augment=None):
        ...
        self.num_vertebra = self.max_points // 4
        self.class_name = ['__background__', 'cell']
        self.num_classes = self.max_points
    ...
    def load_annoFolder(self, img_id):
        labels_dir = os.path.join(self.data_dir, 'labels', self.phase)
        # Keep compatibility with both naming styles: image.jpg.mat and image.mat.
        anno_with_img_ext = os.path.join(labels_dir, img_id + '.mat')
        if os.path.exists(anno_with_img_ext):
            return anno_with_img_ext
        img_stem, _ = os.path.splitext(img_id)
        return os.path.join(labels_dir, img_stem + '.mat')
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'test':
            ...
            if self.augment is None:
                aug_label = self.phase == 'train'
            else:
                aug_label = self.augment
            ...
            data_dict = pre_proc.generate_ground_truth(image=out_image,
                                                       ...
                                                       img_id=img_id,
                                                       num_vertebra=self.num_vertebra)
            return data_dict
