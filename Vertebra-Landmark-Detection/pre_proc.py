...
def processing_test(image, input_h, input_w):
    image, _ = transform.resize_with_letterbox(image, None, (input_w, input_h), fill=0)
...
def generate_ground_truth(image,
    ...
    wh = np.zeros((num_vertebra, 2*4), dtype=np.float32)
    reg = np.zeros((num_vertebra, 2), dtype=np.float32)
    ind = np.zeros((num_vertebra), dtype=np.int64)
    reg_mask = np.zeros((num_vertebra), dtype=np.uint8)

    if pts_2.shape[0] == 0:
        print('ATTENTION!! image {} has empty landmarks.'.format(img_id))
        ret = {'input': image,
               'hm': hm,
               'ind': ind,
               'reg': reg,
    ...
    expected_points = num_vertebra * 4
    if pts_2.shape[0] > expected_points:
        pts_2 = pts_2[:expected_points, :]
    if pts_2.shape[0] != expected_points:
        print('ATTENTION!! image {} pts num {} does not equal expected {}!!! '.format(img_id, pts_2.shape[0], expected_points))

    valid_vertebra = min(num_vertebra, pts_2.shape[0] // 4)
    for k in range(valid_vertebra):
        pts = pts_2[4*k:4*k+4,:]
...
def processing_train(image, pts, image_h, image_w, down_ratio, aug_label, img_id):
    ...
    data_aug = {'train': transform.Compose([transform.ConvertImgFloat(),
                                            ...
                                            transform.RandomAffine(max_rotate=8.0, scale_range=(0.95,1.05), translate_frac=0.03),
                                            transform.Expand(max_scale=1.5, mean=(0, 0, 0)),
                                            transform.RandomGaussianNoise(sigma=3.0),
