...
For the processed cervical spine data in this workspace, use the dedicated spine configs and adjust the epoch count as needed:

```bash
python tools/train.py --cfg experiments/renji/spine_renji_hrnet_w18.yaml
python tools/train.py --cfg experiments/ruijin/spine_ruijin_hrnet_w18.yaml
```

After training, run the spine mm evaluation template:

```bash
python tools/eval_spine.py --cfg experiments/renji/spine_renji_hrnet_w18.yaml \
  --model-file <MODEL_WEIGHT> \
  --resolution-csv ../D-CeLR/data/renji_npy_direct/test_resolution.csv

python tools/eval_spine.py --cfg experiments/ruijin/spine_ruijin_hrnet_w18.yaml \
  --model-file <MODEL_WEIGHT> \
  --resolution-csv ../D-CeLR/data/ruijin_npy/test_resolution.csv
```

The spine configs read the processed `npy` pairs from the existing D-CeLR trees and keep the same Hungarian + spacing metric template used by the other project packages.

The output layout follows the same structured style used by the VLD / D-CeLR runs:

```text
outputs/
  training_renji_spine_renji_hrnet_w18_e{END_EPOCH}/
    config.yaml
    checkpoints/
    logs/
    visualizations/
  training_ruijin_spine_ruijin_hrnet_w18_e{END_EPOCH}/
    config.yaml
    checkpoints/
    logs/
    visualizations/
  inference_renji_spine_renji_hrnet_w18_e{END_EPOCH}/
    config.yaml
    logs/
    predictions/
    visualizations/
    logs/comparison_table.md
  inference_ruijin_spine_ruijin_hrnet_w18_e{END_EPOCH}/
    config.yaml
    logs/
    predictions/
    visualizations/
...
`tools/eval_spine.py` writes the final comparison table to `logs/comparison_table.md` and the raw per-sample metrics to the optional CSV you pass in with `--dump-sample-csv`.
