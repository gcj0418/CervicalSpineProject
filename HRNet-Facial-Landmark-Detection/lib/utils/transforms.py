...
def crop(img, center, scale, output_size, rot=0):
    ...
    if not rot == 0:
        new_img = ndimage.rotate(new_img, rot, reshape=False, order=1, mode='constant', cval=0.0)
    new_img = cv2.resize(new_img, (int(output_size[0]), int(output_size[1])), interpolation=cv2.INTER_LINEAR)
    return new_img
